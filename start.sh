...
docker exec -d tes2_airflow-webserver_1 airflow connections add 'postgres_default' --conn-type 'Postgres' --conn-login 'airflow' --conn-password 'airflow' --conn-host 'localhost' --conn-port 5432 --conn-schema 'airflow'
...
docker exec -d tes2_airflow-webserver_1 airflow connections add 'postgres_Y' --conn-type 'Postgres' --conn-login 'postgres' --conn-password 'postgres' --conn-host 'localhost' --conn-port 1234 --conn-schema 'postgres'
